...
export const ImportRatingsDisplay: React.FunctionComponent<ImportRatingsDisplayProps> =
  ({ ratings }) => {
    const token = useAppSelector((state) => state.token.value);

    const [importRatings, { loading, error, data }] =
      useMutation(IMPORT_RATINGS);

    const downloadRatings = () => {
      const blob = new Blob([ratings]);
